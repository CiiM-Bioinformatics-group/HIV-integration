...
cc <- readxl::read_excel('/vol/projects/CIIM/2000HIV/2000HIV_Flowcytometry/Filtered2_merged_final/2000HIV_FLOW_MFI_panel123merged_QCed_untransformed(raw)data_1434samples_516vars.xlsx',
                          1)
...
fwrite(factors_cc_tests, paste0(outdir,'cc_cor/factors_mfi_test',append,'.csv'), row.names = T)
...
pdf(paste0(outdir,'cc_cor/Factors_mfi_cor',append,'.pdf'),width = 10,height = 40)
Heatmap(cors, show_row_dend = F, cluster_columns = F)
dev.off()

#Only the CCR5 ones
pdf(paste0(outdir,'cc_cor/Factors_mfi_cor_ccr5',append,'.pdf'),width = 10,height = 10)
Heatmap(cors[grepl('MFI_CCR5', rownames(cors)),], show_row_dend = F, cluster_columns = F)
dev.off()
#Only the CXCR4 ones
pdf(paste0(outdir,'cc_cor/Factors_mfi_cor_cxcr4',append,'.pdf'),width = 10,height = 10)
Heatmap(cors[grepl('MFI_CXCR4', rownames(cors)),], show_row_dend = F, cluster_columns = F)
dev.off()
