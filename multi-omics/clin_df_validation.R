...
clin_df <- fread(clin) %>% as.data.frame() %>%
  inner_join(fread(clin2) %>%
               dplyr::rename('Record Id' = `Record.Id`) %>%
...
colnames(clin_df) <- gsub('#', '%', colnames(clin_df))
    ...
    #Maybe MED_STAT%: "Specify statins"
    #Maybe MED_PSY%: "Specify Psychotropic"
    ...
    fwrite(clin_df_clean, paste0(outdir,'validation/clin_df_validation',append,'.csv'), row.names=T)
