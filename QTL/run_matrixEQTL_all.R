...
# this script runs the mapping all at once instead of per phenotype row
# only suitable for data without missing values, otherwise this will be imputed
...
cohort <- "2000HIV-EU-validation"
pheno_type <- "expression"
cov_interact <- "main"
snp_path <- paste(base, "out", cohort, "genotype/dosage/chr22.txt", sep = "/")
...
out_mapping <- paste(base, "out", cohort, pheno_type, "mapping", cov_interact, "chr22.txt", sep = "/")
p_val <- 1
cov_main_unsplit <- "age,sex,bmi,lockdown,hiv_duration,season_cos,season_sin"
index <- FALSE
# all
...
index <- 2
part_length <- 500
out_mapping <- paste(base, "out", cohort, pheno_type, "mapping", cov_interact, "chr22", "parts", "2.tsv", sep = "/")
# parts
...
cov_interact <- args[7]
pheno_type <- args[8]
index <- as.numeric(args[9])
part_length <- as.numeric(args[10])
...
perform_mapping <- function(model, pheno_name, this_pheno, this_lb, this_cov, outfile) {
  ...
  #outfile_cis <- gsub("\\.tsv$", "_cis.tsv", outfile)
  #if (pheno_type == "expression") {
  # load genepos and snpspos
  #  genepos <- fread(file.path(base, "data", cohort, pheno_type, "genpos.tsv"), header = TRUE)
  #  snpspos <- fread(file.path(base, "data", cohort, pheno_type, "snppos.tsv"), header = TRUE)
  #  quest_qtl <- Matrix_eQTL_main(
  #    snps = snps,
  #    gene = pheno,
  #    cvrt = cvrt,
  ...
  #}
  message("Time taken: ", quest_qtl$time.in.sec, " seconds \n")
  #return(quest_qtl)
}

dir.create(dirname(out_mapping), showWarnings = FALSE)
...
if (!is.na(index)) { # take subset
  subset_min <- (index * part_length) + 1
  subset_max <- subset_min + part_length - 1
  message("Taking subset ", index, " (", subset_min, " - ", subset_max, ")")
  pheno_df <- pheno_df[subset_min:subset_max, ]
}
...
pheno_name <- pheno_type
#message("\nNow processing (", i, "/", nrow(pheno_df), "): ", pheno_name)
outfile <- out_mapping #paste0(out_mapping, "/", chr, ".txt")

pheno <- pheno %>%
  #select_if(~ !any(is.na(.))) %>%  #remove without pheno measurement
  dplyr::select(any_of(names(lb_all))) %>%  # select only samples that are genotyped
  dplyr::select(any_of(names(cov_all)))  # only samples that have covariate info
message("Sample change after matching samples: ", orig_len, " -> ", ncol(pheno), "\n")

# save file
#fwrite(pheno, file = "~/test.csv", sep = ",", col.names = TRUE, row.names = TRUE, quote = FALSE)
...
if (chr == "chr1" && cov_interact == "main") {
  ...
  fwrite(info_df, file = paste0(dirname(out_mapping), "/mapped_samples.txt"), sep = "\t")
  message("Saved sample info in ", paste0(dirname(out_mapping), "/mapped_samples.txt"))
}
...
if (ncol(pheno) <= (nrow(cov_all) + 1 + 1)) {
  ...
  if (cov_interact == "main") { # aka no interaction
    ...
    if (any(cov_rows)) {  
      ...
      perform_mapping(modelLINEAR, pheno_name, pheno, lb, cov_only_main, outfile)
    }
  } else { 
    ...
    if (any(cov_rows)) {
      ...
      perform_mapping(modelLINEAR_CROSS, pheno_name, pheno_this, lb_this, cov_this, outfile)
    }
  }
}